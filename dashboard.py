    ...
    def chat_fn(message: str, chat_id: str = "", attachments: list | None = None) -> dict:
        ...
        with lock:
            ...
            if chat.get("mode") == "team":
                ...
                def _said(row):
                    # Una nota di openvurp (chi e' fuori, chi non ha risposto)
                    # non e' un intervento: va mostrata come avviso, subito.
                    if row.get("author_type") == "system":
                        _act.publish(
                            "room_note", source="dashboard", chat_id=chat_id,
                ...
                try:
                    ...
                    # Una stanza caduta a meta' lasciava il run «in corso» per
                    # sempre e la pagina su «stanno discutendo» finche' la
                    # scopa dei cinque minuti non la puliva.
                    ...
                    try:
                        ...
                        _act.publish("room_end", source="dashboard", chat_id=chat_id,
                                     session_key=room_key, reason="error", rounds=0)
                    except Exception:
                        pass
                    raise
                _mp.clear_stop(chat_id)
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/whatsapp/status":
            ...
            return self._json_response({
                ...
                "qr": ch.qr, "me": ch.me, "error": ch.error,
            })
        if path == "/api/file":
    ...
    def do_POST(self):
        ...
        attachments = [str(a) for a in raw_attachments if str(a).strip()] \
            if isinstance(raw_attachments, list) else []
        if len(attachments) > 8:
            # Before, the ninth file was dropped without a word.
            return self._json_response(
                {"error": f"at most 8 files per message (you sent "
                          f"{len(attachments)})"}, status=400)
    ...
    SETTABLE = (
        ...
        "MULTIPLAYER_MAX_ROUNDS", "MULTIPLAYER_MAX_AGENTS",
        "MULTIPLAYER_DAILY_CALL_BUDGET",
    )
    ...
    def _serve_preview(self):
        ...
        if not vero:
            # «C'e'?» e' una domanda, non un errore: chi la fa vuole sapere se
            # aprire, e un 404 lo mandava a mostrare una scheda vuota.
            if params.get("as") == "exists":
        ...
        if params.get("as") == "exists":
            return self._json_response({"exists": True, "name": nome,
                                        "size": dimensione})
